...
class EventoArtista implements IBaseModelo{
    private $id;
    private $idEvento;
    private $idServico;
    private $conn;
    ...
    public function inserir(){
        try{
            ...
            $query="INSERT INTO evento_artista (idEvento, idServico) VALUES (:idEvento, :idServico) ";
    ...
    public function alterar($param){
        try{
            ...
            //Comando SQL para inserir um aluno
            $query="UPDATE evento_artista SET idEvento = :idEvento, idServico = :idServico WHERE id=:id ";
            $this->stmt= $this->conn->prepare($query);
    ...
    public function listarTodos($evento, $idUsuario){
        ...
        try{
            ...
            if(!is_null($evento)){
                //Pesquisa pelo nome
                $query="SELECT id,idEvento,idServico FROM evento_artista WHERE idEvento = :evento";
            }else{
                // Pesquisa todos
                $query="SELECT id,idEvento,idServico FROM evento_artista";
            }
            $this->stmt= $this->conn->prepare($query);
            ...
            if($this->stmt->execute()){
                ...
                $eventos = $this->stmt->fetchAll(PDO::FETCH_CLASS,"EventoArtista");  
    ...
    public function listarUnico($id){
        ...
        try{
            $query="SELECT id,idEvento,idServico FROM evento_artista WHERE id=:id";
            $this->stmt= $this->conn->prepare($query);
            $this->stmt->bindValue(':id', $id, PDO::PARAM_INT);
            
            if($this->stmt->execute()){
                // Associa o registro a uma classe aluno
                $evento = $this->stmt->fetchAll(PDO::FETCH_CLASS,"EventoArtista");  
